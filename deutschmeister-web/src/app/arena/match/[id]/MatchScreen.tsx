  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      ...
      useArenaStore.getState().applyError({
        code: 'INTERNAL',
        message: 'Không nhận được phản hồi từ server. Vui lòng thử lại.',
      });
    }, 5_000);
    return () => clearTimeout(t);
  }, [forfeitting]);
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        <Card className="text-center py-12">
          <div className="text-lead mb-2" style={{ color: 'var(--theme-text-primary)' }}>
            {lastError ? 'Không thể tải trận đấu' : 'Đang kết nối tới trận đấu...'}
          </div>
          <div className="text-caption" style={{ color: 'var(--theme-text-muted)' }}>
            {lastError?.message ?? 'Nếu không tải được, trận có thể đã kết thúc hoặc bị hủy.'}
          </div>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        ...
        <Card className="text-center py-10">
          ...
          <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-4">
            <PlayerIntro name={match.me.displayName} level={match.me.level} label="Bạn" />
            ...
            <PlayerIntro
              ...
              label={match.opponent.isBot ? 'Bot' : 'Đối thủ'}
            />
          </div>
          <div
            key={countdownLeft}
      ...
      {confirmForfeit && (
        ...
        >
          ...
          >
            ...
            >
              Rời trận ngay bây giờ?
            </h3>
            <p
              className="text-body text-center mb-4"
              style={{ color: 'var(--theme-text-secondary)' }}
            >
              Bạn sẽ <strong style={{ color: STATUS.danger }}>thua trận</strong>. Tất cả vòng còn lại sẽ được tính cho đối thủ.
              {match.opponent.isBot
                ? ''
                : ' Đây là trận xếp hạng — rating của bạn sẽ giảm.'}
            </p>
      ...
      <form onSubmit={handleSubmit}>
        <div className="flex items-center gap-2">
          <input
            ...
            placeholder={currentClue.mode === 'vi_to_de' ? 'Nhập từ tiếng Đức...' : 'Nhập nghĩa tiếng Việt...'}
        ...
        <div className="mt-2 flex flex-wrap items-center gap-2">
          ...
          <span className="text-caption" style={{ color: 'var(--theme-text-muted)' }}>
            {currentClue.mode === 'vi_to_de'
              ? 'Có thể nhập ae/oe/ue/ss thay cho ä/ö/ü/ß.'
              : 'Có thể nhập tiếng Việt không dấu.'}
          </span>
        </div>
      </form>
        ...
        {myLastAnswer && phase === 'playing' && (
          ...
          >
            {myLastAnswer.isCorrect
              ? 'Đúng! Đợi đối thủ hoặc hết giờ.'
              : `Sai (lần ${myLastAnswer.attempts}). Thử lại!`}
          </div>
        )}
        {opponentLastResult && phase === 'playing' && (
          ...
          >
            {opponentLastResult.isCorrect
              ? `Đối thủ trả lời đúng (${Math.round((opponentLastResult.responseTimeMs ?? 0) / 1000)}s).`
              : `Đối thủ trả lời sai (lần ${opponentLastResult.attempts}).`}
          </div>
        )}
        {opponentTyping && phase === 'playing' && (
          <div className="text-caption" style={{ color: 'var(--theme-text-muted)' }}>
            {match.opponent.displayName} đang gõ...
          </div>
        )}
      </div>
      ...
      >
        ...
        <div className="flex items-center justify-center gap-3 mt-4">
          {ratingTarget !== 0 && (
            ...
            >
              {ratingSign}{Math.abs(ratingShown)} rating
            </div>
          )}
          {xpTarget > 0 && (
            <div
        ...
        <div className="text-caption mt-4" style={{ color: 'var(--theme-text-muted)' }}>
          Đang chuyển sang trang kết quả...
        </div>
      </div>
    </div>
  );

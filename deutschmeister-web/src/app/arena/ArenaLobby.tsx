...
const MODES: { key: ArenaMode; label: string; sub: string; badge: string; gradient: string }[] = [
  {
    key: 'vi_to_de',
    label: 'Việt → Đức',
    sub: 'Nhìn nghĩa tiếng Việt, nhập từ tiếng Đức.',
    badge: 'VI→DE',
  ...
  },
  {
    key: 'de_to_vi',
    label: 'Đức → Việt',
    sub: 'Nhìn từ tiếng Đức, nhập nghĩa tiếng Việt.',
    badge: 'DE→VI',
    gradient: GRADIENT.examWriting,
  },
  {
    key: 'mixed',
    label: 'Hỗn hợp',
    sub: 'Xen kẽ hai hướng trong cùng một trận.',
    badge: 'MIX',
...
export default function ArenaLobby() {
  ...
  const queueCopy = useMemo(() => {
    ...
    if (elapsed >= fallbackMs) return 'Đang chuẩn bị đối thủ bot để bạn không phải chờ lâu.';
    if (elapsed >= ARENA_EXPANSION_2_MS) return 'Đang mở rộng tìm kiếm sang nhiều cấp độ hơn.';
    if (elapsed >= ARENA_EXPANSION_1_MS) return 'Đang mở rộng tìm kiếm sang cấp độ lân cận.';
    return 'Đang tìm đối thủ cùng trình độ và cùng chế độ.';
  }, [elapsed, queueEstimatedWaitMs]);

  const connectionCopy =
    connectionStatus === 'connected'
      ? null
      : connectionStatus === 'connecting'
        ? 'Đang kết nối tới Đấu trường...'
        : connectionStatus === 'reconnecting'
          ? 'Đang kết nối lại. Trận sẽ tự khôi phục nếu còn hiệu lực.'
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-5">
          <div className="flex-1">
            ...
            <p className="text-body mt-1 opacity-90">
              10 vòng, 30 giây mỗi vòng. Ai trả lời đúng trước sẽ giành điểm.
            </p>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 min-w-0 sm:min-w-[360px]">
            {statsLoading ? (
              ...
              <>
                <Stat label="Rating" value={stats?.rating ?? 1000} />
                <Stat label="Win rate" value={formatWinRate(stats)} />
                <Stat label="W-L-D" value={`${stats?.wins ?? 0}-${stats?.losses ?? 0}-${stats?.draws ?? 0}`} />
      ...
      {replaced && (
        <Alert tone="warning" className="mb-4">
          Phiên đấu trường đã chuyển sang tab khác. Bạn đang ở tab cũ.
        </Alert>
      )}
      ...
      {!isQueueing ? (
        ...
        <Button onClick={leaveQueue} variant="outline" size="lg" fullWidth>
          Hủy tìm trận ({Math.floor(elapsed / 1000)}s)
        </Button>
      )}
...
function RecentMatchesCard({
  ...
  return (
    <SectionCard title="Lịch sử gần đây" href="/arena/history" hrefLabel="Xem tất cả">
      {loading ? (
        ...
        <div className="text-caption text-center py-6" style={{ color: 'var(--theme-text-muted)' }}>
          Chưa có trận nào. Bấm &ldquo;Tìm trận&rdquo; để bắt đầu!
        </div>
      ) : (
...
function TopPlayersCard({
  ...
  return (
    <SectionCard title="Top xếp hạng" href="/arena/leaderboard" hrefLabel="Xem tất cả">
      {loading ? (
        <div className="flex justify-center py-6"><Loading /></div>
      ) : items.length === 0 ? (
        <div className="text-caption text-center py-6" style={{ color: 'var(--theme-text-muted)' }}>
          Chưa có ai trên bảng xếp hạng — bạn có thể là người đầu tiên!
        </div>
      ) : (
        <div className="space-y-2">
          {items.map((r, i) => {
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-caption" style={{ color: 'var(--theme-text-muted)' }}>
                    {r.wins}W · {r.losses}L · streak {r.currentStreak}
                  </div>
                </div>
                <div

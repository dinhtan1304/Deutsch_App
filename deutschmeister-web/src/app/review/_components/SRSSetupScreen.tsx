...
export function SRSSetupScreen({ stats, quizMode, onSetMode, onStart, onBack, onAddWords }: SRSSetupScreenProps) {
  const statItems = [
    { label: 'Cần ôn', value: stats?.due ?? 0,      icon: IconFlame,    color: STATUS.danger,   bg: `linear-gradient(135deg, ${STATUS.danger}1F, ${STATUS.danger}0F)` },
    { label: 'Đã thuộc', value: stats?.mastered ?? 0, icon: IconTarget,   color: STATUS.success,  bg: `linear-gradient(135deg, ${STATUS.success}1F, ${STATUS.success}0F)` },
    { label: 'Đang học', value: stats?.learning ?? 0, icon: IconBookOpen, color: ACCENT.xp,       bg: `linear-gradient(135deg, ${ACCENT.xp}1F, ${ACCENT.xp}0F)` },
    { label: 'Tổng',     value: stats?.total ?? 0,    icon: IconBrain,    color: ACCENT.writing,  bg: `linear-gradient(135deg, ${ACCENT.writing}1F, ${ACCENT.writing}0F)` },
  ];
        ...
        {(stats?.due ?? 0) > 0 ? (
          <>
            ...
            <p className="text-sm mb-6" style={{ color: 'var(--theme-text-secondary)' }}>
              Bạn có <span className="font-bold" style={{ color: ACCENT.srs }}>{stats?.due} từ</span> cần ôn tập hôm nay
            </p>

...
function HistoryCard({ item, onDelete }: { item: ExamReadingHistoryItem; onDelete: () => void }) {
  ...
  return (
    <Link href={isGraded ? `/practice-test/reading/exam/${item.id}/result` : `/practice-test/reading/exam/${item.id}`}
      className="group block rounded-[2.5rem] p-6 transition-all duration-500 hover:-translate-y-1.5 hover:shadow-2xl relative overflow-hidden border border-transparent hover:border-green-500/20"
      style={{
      ...
      <div className="relative z-10 flex items-center gap-6">
        <div className="w-16 h-16 rounded-2xl flex items-center justify-center shrink-0 shadow-lg transition-all duration-500 group-hover:scale-110 group-hover:rotate-3"
          style={{
            background: isGraded ? GRADIENT.reading : 'var(--theme-bg-secondary)',
            color: isGraded ? 'white' : ACCENT.reading,
            boxShadow: isGraded ? '0 10px 20px rgba(34, 197, 94, 0.2)' : 'none',
          }}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 mb-2.5 flex-wrap">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-lg"
              style={{
                backgroundColor: isGraded ? `${STATUS.success}15` : `${ACCENT.writing}15`,
...
export default function ExamReadingListPage() {
  ...
  const { data: history, isLoading } = useExamReadingHistory({
    page, limit: 10,
    ...
    <div className="max-w-5xl mx-auto px-4 py-8 pb-32">
      ...
      {stats && stats.total > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 mb-16">
          {[
            { label: 'Tổng bài thi', value: stats.total, color: ACCENT.reading, icon: <IconBookOpen size={28} /> },
      ...
      <div className="flex flex-col gap-5 mb-10">
        <div className="flex gap-3 overflow-x-auto no-scrollbar pb-1">
          {[
            ...
            const color = s.id === 'GRADED' ? STATUS.success : s.id === 'DRAFT' ? ACCENT.games : ACCENT.reading;
            return (
      ...
      {isLoading ? (
        ...
        <div className="text-center py-28 rounded-[3.5rem] border-2 border-dashed" style={{ borderColor: 'var(--theme-border)' }}>
          ...
          <p className="text-base opacity-50 mb-10 max-w-xs mx-auto font-medium">Bắt đầu làm bài luyện đọc theo đề chuẩn Goethe/TELC ngay nhé!</p>
          <Link href="/practice-test/reading/exam/new"

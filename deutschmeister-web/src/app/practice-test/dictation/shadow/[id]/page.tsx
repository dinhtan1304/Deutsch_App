  ...
  return (
    ...
    >
      ...
      <p className="flex-1 text-sm font-medium leading-relaxed" style={{ color: 'var(--theme-text-secondary)' }}>
        Tạm dừng và đọc to. Tập trung vào nhịp điệu và phát âm, không cần hoàn hảo. Bắt đầu chậm rồi tăng tốc dần.
      </p>
      <button
        type="button"
        onClick={onDismiss}
...
export default function ShadowingPlayPage() {
  ...
  async function handleRecord(audioBase64: string, mimeType: string) {
    ...
    try {
      ...
      setRecordError(err instanceof Error ? err.message : 'Chấm điểm thất bại.');
    } finally {
  ...
  function handleSubmit() {
    ...
    if (session.completedSegments === 0) {
      setSubmitWarning('Hãy luyện ít nhất 1 câu trước khi hoàn thành.');
      return;
    }
    setSubmitWarning('');
    submit(id, {
      onSuccess: () => router.push(`/practice-test/dictation/shadow/${id}/result`),
      onError: (err: Error) => setSubmitWarning(err.message || 'Không thể hoàn tất phiên.'),
    });
  }
    ...
    return (
      <div className="py-20 text-center">
        <p className="text-sm font-medium" style={{ color: 'var(--theme-text-muted)' }}>
          Không tìm thấy phiên shadowing.
        </p>
      </div>
    );
  }
          ...
          <div className="flex flex-col md:flex-row gap-5 mb-6">
            ...
            <div className="md:w-72 lg:w-80 shrink-0">
              ...
              <div className="mt-2 flex items-center gap-2">
                ...
                >
                  {session.video.transcriptSource === 'ai' ? '✨ Tạo bởi AI' : 'Lấy từ Youtube'}
                </span>
              </div>
              {/* Speed + auto-pause */}
              ...
              {isProcessing ? (
                ...
                <>
                  ...
                  <p
                    ...
                    style={{ color: 'var(--theme-text-muted)' }}
                  >
                    Nên dùng tai nghe để đạt hiệu quả tốt hơn
                  </p>
                </>
              )}
          ...
          <div className="mt-6 flex items-center justify-between gap-4 flex-wrap">
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between text-[11px] font-black mb-1.5">
                <span style={{ color: 'var(--theme-text-secondary)' }}>Tiến độ</span>

...
type ExamKey = 'goethe' | 'telc';
type LevelKey = 'a1' | 'a2' | 'b1';

const EXAM_OPTIONS = [
  { id: 'GOETHE', tKey: 'goethe' as ExamKey, levels: ['A1', 'A2', 'B1'], color: STATUS.success },
  { id: 'TELC', tKey: 'telc' as ExamKey, levels: ['A2', 'B1'], color: ACCENT.srs },
];

const LEVEL_KEYS: Record<string, LevelKey> = { A1: 'a1', A2: 'a2', B1: 'b1' };

export default function StudyPlanSetup() {
  const t = useTranslations('studyPlan.setup');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError((err as Error | undefined)?.message || t('step2.errorDefault'));
    }
  };
      ...
      <div className="w-full max-w-lg">
        ...
        <div className="text-center mb-10">
          ...
          <div className="text-[10px] font-black uppercase tracking-[0.2em] opacity-40 mb-1" style={{ color: 'var(--theme-text-primary)' }}>
            {t('stepLabel', { current: step + 1, total: 3 })}
          </div>
        </div>
                  ...
                  <div className="flex items-center gap-5 relative z-10">
                    ...
                    <div className="flex-1">
                      <div className="font-black text-lg tracking-tight" style={{ color: 'var(--theme-text-primary)' }}>
                        {t(`exams.${exam.tKey}.name` as 'exams.goethe.name')}
                      </div>
                      <div className="text-sm font-medium mt-1 opacity-70" style={{ color: 'var(--theme-text-muted)' }}>
                        {t(`exams.${exam.tKey}.description` as 'exams.goethe.description')}
                      </div>
          ...
          <div className="space-y-6" style={{ animation: 'slideUp .4s cubic-bezier(0.16, 1, 0.3, 1)' }}>
            <div className="text-center">
              ...
              <p className="text-sm font-medium" style={{ color: 'var(--theme-text-muted)' }}>
                {targetLevel ? t('step1.subtitle', { level: targetLevel }) : t('step1.subtitleEmpty')}
              </p>
            </div>

            <div className="grid gap-4">
              {selectedExam.levels.map((level) => {
                const lk = LEVEL_KEYS[level] ?? 'a1';
        ...
        {step === 2 && (
          <div className="space-y-6" style={{ animation: 'slideUp .4s cubic-bezier(0.16, 1, 0.3, 1)' }}>
            <div className="text-center">
              ...
              <p className="text-sm font-medium" style={{ color: 'var(--theme-text-muted)' }}>
                {t('step2.subtitle', { exam: examFormat, level: targetLevel })}
              </p>
            </div>
                ...
                <div className="p-5 rounded-2xl flex items-start gap-4 transition-all"
                  ...
                  <div>
                    <div className="font-black text-lg" style={{ color: weeksUntilExam < 8 ? STATUS.danger : ACCENT.srs }}>
                      {t('step2.weeksLeft', { weeks: weeksUntilExam })}
                    </div>
            ...
            <div className="pt-2">
              ...
              >
                {createPlan.isPending ? t('step2.submitting') : t('step2.submit')}
              </button>

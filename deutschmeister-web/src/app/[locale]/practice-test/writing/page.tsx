...
// ─── Writing-type metadata (real writingType → color var + emoji) ────────────
const TYPE_COLOR: Record<string, string> = {
  email: 'var(--der)', brief: 'var(--die)', beschreibung: 'var(--violet)', tagebuch: 'var(--cyan)',
  dialog: 'var(--accent)', aufsatz: 'var(--warn)', einladung: 'var(--success)', beschwerde: 'var(--danger)',
  bewerbung: 'var(--der)', formular: 'var(--theme-text-muted)',
};
const TYPE_ICON: Record<string, string> = {
  email: '✉', brief: '📨', beschreibung: '🖼', tagebuch: '📔', dialog: '💬',
  aufsatz: '📝', einladung: '🎉', beschwerde: '😠', bewerbung: '📑', formular: '📋',
};
const KNOWN_TYPES = Object.keys(TYPE_COLOR);

const STATUS_COLOR: Record<string, string> = {
  DRAFT: 'var(--theme-text-muted)', SUBMITTED: 'var(--cyan)', GRADING: 'var(--warn)',
  GRADED: 'var(--success)', ERROR: 'var(--danger)',
};
...
export default function WritingListPage() {
  ...
  const t = useTranslations('practice.writing.list');
  ...
  const typeLabel = (ty: string) => KNOWN_TYPES.includes(ty) ? t(`writingTypes.${ty}` as 'writingTypes.email') : ty;
  const statusLabel = (s: string) => {
    if (s === 'DRAFT') return tCommon('status.draftWriting');
    if (s === 'GRADING') return tCommon('status.gradingShort');
    return tCommon(`status.${s}` as 'status.GRADED');
  };

  const allItems = useMemo(() => history?.data ?? [], [history]);
  // Only surface type chips for types actually present (writing has 10 types).
  const presentTypes = useMemo(() => KNOWN_TYPES.filter((ty) => allItems.some((i) => i.writingType === ty)), [allItems]);
  const items = useMemo(() => (filterType ? allItems.filter((i) => i.writingType === filterType) : allItems), [allItems, filterType]);
    ...
    <div className="mx-auto max-w-360 px-4 py-6 sm:px-6">
      ...
      {inProgress ? (
        <section className="v2-hero-warn mb-6 flex flex-col gap-4 overflow-hidden rounded-2xl p-5 sm:flex-row sm:items-center"
          ...
          <div className="v2-icongrad-warn flex h-12 w-12 shrink-0 items-center justify-center rounded-[13px] text-white" style={{ boxShadow: '0 8px 20px color-mix(in srgb, var(--warn) 40%, transparent)' }}>
            <IconPenLine size={24} style={{ color: 'white' }} />
          </div>
          <div className="min-w-0 flex-1">
            <span className="text-[11px] font-bold uppercase" style={{ color: 'var(--warn)', letterSpacing: '.05em' }}>{tHub('heroContinueEyebrow')}</span>
            <h3 className="mt-0.5 truncate text-h3 font-bold" style={{ color: 'var(--theme-text-primary)' }}>{inProgress.topic}</h3>
            <p className="text-caption" style={{ color: 'var(--theme-text-muted)' }}>{typeLabel(inProgress.writingType)} · {inProgress.cefrLevel}</p>
          </div>
          <div className="flex shrink-0 flex-wrap items-center gap-2">
            <Link href={`/practice-test/writing/${inProgress.id}`} className="inline-flex h-10 items-center gap-1.5 rounded-[10px] px-4 text-caption font-bold text-white transition-transform hover:-translate-y-0.5" style={{ background: 'var(--warn)', boxShadow: '0 4px 14px color-mix(in srgb, var(--warn) 40%, transparent)' }}>
              {tHub('heroContinueCta')} <IconArrowRight size={14} />
            </Link>
            <Link href="/practice-test/writing/new" className="v2-btn-soft inline-flex h-10 items-center rounded-[10px] px-4 text-caption font-bold" style={{ background: 'var(--theme-bg-card)', color: 'var(--theme-text-secondary)', border: '1px solid var(--theme-border)' }}>
              {tHub('newCta')}
            </Link>
            <Link href="/practice-test/writing/exam" className="v2-btn-soft inline-flex h-10 items-center rounded-[10px] px-4 text-caption font-medium" style={{ background: 'transparent', color: 'var(--theme-text-muted)', border: '1px solid var(--theme-border)' }}>
              {tHub('examCta')}
            </Link>
          </div>
        </section>
        ...
        <section className="v2-hero-accent mb-6 flex flex-col items-start gap-4 overflow-hidden rounded-2xl p-5 sm:flex-row sm:items-center"
          ...
          <div className="flex shrink-0 gap-2">
            <Link href="/practice-test/writing/new" className="inline-flex h-10 items-center gap-1.5 rounded-[10px] px-5 text-caption font-bold text-white transition-transform hover:-translate-y-0.5" style={{ background: 'var(--accent)', boxShadow: '0 4px 14px color-mix(in srgb, var(--accent) 45%, transparent)' }}>
              {tHub('newCta')} <IconArrowRight size={14} />
            </Link>
            <Link href="/practice-test/writing/exam" className="v2-btn-soft inline-flex h-10 items-center rounded-[10px] px-4 text-caption font-medium" style={{ background: 'var(--theme-bg-card)', color: 'var(--theme-text-secondary)', border: '1px solid var(--theme-border)' }}>
              {tHub('examCta')}
            </Link>
          </div>
        </section>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-x-4 gap-y-3 rounded-xl p-3" style={{ background: 'var(--theme-bg-card)', border: '1px solid var(--theme-border)' }}>
        ...
        <FilterGroup label={tHub('filterStatus')}>
          ...
          {['DRAFT', 'SUBMITTED', 'GRADING', 'GRADED'].map((s) => (
            <Chip key={s} label={statusLabel(s)} on={filterStatus === s} onClick={() => { setFilterStatus(s); setPage(1); }} dotColor={STATUS_COLOR[s]} />
          ))}
        </FilterGroup>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-2xl border border-dashed py-20 text-center" style={{ borderColor: 'var(--theme-border)' }}>
          ...
          <Link href="/practice-test/writing/new" className="inline-flex items-center gap-2 rounded-xl px-6 py-3 text-body font-bold text-white" style={{ background: GRADIENT.writing }}>{t('emptyCta')}</Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((item) => {
            const tColor = TYPE_COLOR[item.writingType] ?? 'var(--accent)';
            const sColor = STATUS_COLOR[item.status] ?? 'var(--theme-text-muted)';
            const isGraded = item.status === 'GRADED';
            const targetHref = isGraded ? `/practice-test/writing/${item.id}/result` : `/practice-test/writing/${item.id}`;
            return (
                ...
                <article className="word-card-v2 flex h-full flex-col gap-2.5 rounded-[13px] p-4"
                  ...
                  <div className="flex flex-wrap items-center gap-1.5">
                    <span className="inline-flex items-center gap-1.5 rounded-md px-2 py-0.5 text-[11px] font-semibold" style={{ background: `color-mix(in srgb, ${tColor} 16%, transparent)`, color: tColor }}>
                      <span className="text-caption">{TYPE_ICON[item.writingType] ?? '📝'}</span>{typeLabel(item.writingType)}
                    </span>
                    ...
                    <span className="inline-flex items-center gap-1.5 rounded-[5px] px-2 py-0.5 text-[10px] font-bold uppercase" style={{ background: `color-mix(in srgb, ${sColor} 16%, transparent)`, color: sColor, letterSpacing: '.04em' }}>
                      <span className="h-1 w-1 rounded-full" style={{ background: sColor }} />{statusLabel(item.status)}
                    </span>
                  </div>
                  <h3 className="text-body font-bold leading-snug" style={{ letterSpacing: '-.01em', color: 'var(--theme-text-primary)' }}>{item.topic}</h3>
                  {item.overallScore !== null && (
                    <div>
                      <span className="mono inline-flex items-baseline gap-0.5 rounded-lg px-2.5 py-1 text-body font-extrabold" style={{ background: `color-mix(in srgb, ${scoreColor(item.overallScore)} 14%, transparent)`, color: scoreColor(item.overallScore) }}>
                        {Math.round(item.overallScore)}<span className="text-[11px]">%</span>
                      </span>
                    </div>
                  )}
                  <div className="mt-auto flex items-center justify-between gap-2 border-t border-dashed pt-2.5" style={{ borderColor: 'var(--theme-border)' }}>
                    <span className="mono inline-flex items-center gap-1 text-[11px] opacity-70" style={{ color: 'var(--theme-text-muted)' }}><IconClock size={11} />{formatDate(item.createdAt)}</span>
                    <div className="flex items-center gap-1.5">
                      <span className="inline-flex items-center gap-1 text-[11px] font-bold" style={{ color: tColor }}>{isGraded ? tHub('view') : tHub('continue')}<IconArrowRight size={12} /></span>

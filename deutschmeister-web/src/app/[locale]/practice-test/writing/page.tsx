...
// Color-only writing type lookup. Labels come from i18n.
const WRITING_TYPE_COLOR: Record<string, string> = {
  email:         ACCENT.srs,
  brief:         ACCENT.vocab,
  beschreibung:  ACCENT.writing,
  tagebuch:      ACCENT.listening,
  dialog:        ACCENT.teal,
  aufsatz:       ACCENT.xp,
  einladung:     STATUS.success,
  beschwerde:    STATUS.danger,
  bewerbung:     STATUS.info,
  formular:      'var(--theme-text-muted)',
};

const KNOWN_WRITING_TYPES = Object.keys(WRITING_TYPE_COLOR);

const STATUS_VISUAL: Record<string, { color: string; bg: string }> = {
  DRAFT:     { color: 'var(--theme-text-muted)', bg: 'rgba(107,114,128,.1)' },
  SUBMITTED: { color: STATUS.info,               bg: 'rgba(59, 130, 246, 0.1)' },
  GRADING:   { color: STATUS.warning,            bg: 'rgba(245, 158, 11, 0.1)' },
  GRADED:    { color: STATUS.success,            bg: 'rgba(34, 197, 94, 0.1)' },
  ERROR:     { color: STATUS.danger,             bg: 'rgba(239, 68, 68, 0.1)' },
};
...
export default function WritingListPage() {
  ...
  const formatDate = (dateStr: string) =>
    formatter.dateTime(new Date(dateStr), {
      day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit',
    });

  // Resolve writing-type label via i18n; unknown types fall back to the raw value.
  const labelForWritingType = (type: string): string => {
    if (KNOWN_WRITING_TYPES.includes(type)) {
      return t(`writingTypes.${type}` as 'writingTypes.email');
    }
    return type;
  };

  // Status badge label resolved against `practice.common.status` — DRAFT in
  // writing context reads "Nháp" not "Chưa làm", so we use the writing-specific
  // key `draftWriting`.
  const labelForStatus = (status: string): string => {
    if (status === 'DRAFT') return tCommon('status.draftWriting');
    if (status === 'SUBMITTED') return tCommon('status.SUBMITTED');
    if (status === 'GRADING') return tCommon('status.GRADING');
    if (status === 'GRADED') return tCommon('status.GRADED');
    if (status === 'ERROR') return tCommon('status.ERROR');
    return status;
  };
    ...
    >
      ...
      {stats && stats.totalSessions > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          {[
            { label: t('stats.totalSessions'), value: stats.totalSessions, color: ACCENT.writing, icon: <IconPenLine size={20} /> },
            { label: t('stats.averageScore'), value: stats.averageScore ? `${Math.round(stats.averageScore)}%` : '—', color: ACCENT.xp, icon: <IconDice size={20} /> },
            { label: t('stats.bestScore'), value: stats.totalErrors ? `${Math.round(stats.totalErrors)}%` : '—', color: STATUS.success, icon: <IconCheck size={20} /> },
          ].map((s, i) => (
      ...
      <div className="flex flex-col gap-5 mb-10">
        ...
        <div className="flex gap-3 overflow-x-auto no-scrollbar pb-1">
          {[
            ...
            { id: 'DRAFT', label: tCommon('status.draftWriting') },
            { id: 'SUBMITTED', label: tCommon('status.SUBMITTED') },
            { id: 'GRADING', label: tCommon('status.gradingShort') },
            { id: 'GRADED', label: tCommon('status.GRADED') },
          ].map(status => {
            const isActive = filterStatus === status.id;
            const visual = STATUS_VISUAL[status.id];
            const color = status.id === '' ? ACCENT.writing : visual?.color || ACCENT.writing;
            return (
      ...
      {isLoading ? (
        ...
        <div className="text-center py-28 rounded-[3.5rem] border-2 border-dashed" style={{ borderColor: 'var(--theme-border)' }}>
          ...
          <Link href="/practice-test/writing/new" className="inline-flex items-center gap-2 px-10 py-5 rounded-2xl text-base font-black text-white shadow-2xl shadow-indigo-500/30 transition-all hover:scale-105 active:scale-95"
            style={{ background: GRADIENT.writing }}>{t('emptyCta')}</Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-8">
          {history.data.map(item => {
            const typeLabel = labelForWritingType(item.writingType);
            // Fallback chain is exhaustive at runtime (DRAFT is always defined),
            // but noUncheckedIndexedAccess can't see that — assert non-null.
            const statusVisual = (STATUS_VISUAL[item.status] ?? STATUS_VISUAL.DRAFT)!;
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-lg"
                        style={{ backgroundColor: statusVisual.bg, color: statusVisual.color }}>{statusLabel}</span>
                      <span className="text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-lg border bg-black/3 dark:bg-white/5"
                        style={{ borderColor: 'var(--theme-border)', color: 'var(--theme-text-muted)' }}>{typeLabel}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-3 text-[11px] font-bold opacity-40 uppercase tracking-widest" style={{ color: 'var(--theme-text-primary)' }}>
                      <span>{t('typeLabel', { type: typeLabel })}</span>
                      <span className="w-1 h-1 rounded-full bg-current" />
                      {item.status === 'GRADED' && (
                        <>
                          <span className="text-emerald-500">{t('gradedLabel')}</span>
                          <span className="w-1 h-1 rounded-full bg-current" />
                        </>
                      )}

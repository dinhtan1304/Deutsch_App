      ...
      <button onClick={() => setOpen(v => !v)}
        ...
        <div className="flex-1 min-w-0">
          <p className="text-body font-semibold leading-snug" style={{ color: 'var(--theme-text-primary)' }}>
            {t('questionLabel', { index: index + 1 })} {question.questionText}
          </p>
          {!isCorrect && (
            <p className="text-xs mt-0.5" style={{ color: STATUS.danger }}>
              {t('correctAnswer', { answer: correctOpt?.text || question.correctAnswer })}
            </p>
          )}
        </div>
        ...
        <div className="px-4 pb-4 border-t space-y-3"
          ...
          {detail?.explanationVi && (
            <div className="rounded-xl p-3" style={{ backgroundColor: `${ACCENT.reading}12` }}>
              <p className="text-caption font-bold mb-1" style={{ color: ACCENT.reading }}>{t('explanation')}</p>
...
export default function ReadingResultPage() {
  ...
  const stats: StatItem[] = [
    { icon: <span style={{ fontSize: 18 }}>✓</span>, label: t('stats.correct'), value: correct, accent: 'reading' },
    { icon: <span style={{ fontSize: 18 }}>✗</span>, label: t('stats.wrong'), value: wrong },
    { icon: <span style={{ fontSize: 18 }}>⏱</span>, label: t('stats.time'), value: timeMs !== null ? formatTime(timeMs) : '—', accent: 'srs' },
    { icon: <span style={{ fontSize: 18 }}>★</span>, label: t('stats.score'), value: `${Math.round(scorePct)}%`, accent: 'xp' },
  ];
        ...
        <div className="flex flex-col sm:flex-row">
          <div className="sm:w-1/2 flex flex-col items-center justify-center gap-4 py-6 px-4 border-b sm:border-b-0 sm:border-r"
            ...
            <div className="flex items-center gap-2 px-4 py-1.5 rounded-full text-body font-bold"
              ...
              <span>{t(`grades.${grade.key}` as 'grades.excellent')}</span>
            </div>
          </div>

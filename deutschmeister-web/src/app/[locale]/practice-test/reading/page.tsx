...
export default function ReadingListPage() {
  ...
  const STATUS_LABEL_KEY: Record<string, 'DRAFT' | 'GRADED'> = {
    DRAFT: 'DRAFT',
    GRADED: 'GRADED',
  };
  const STATUS_CONFIG_VISUAL: Record<string, { color: string; bg: string }> = {
    DRAFT: { color: 'var(--theme-text-muted)', bg: 'rgba(107, 114, 128, 0.1)' },
    ...
    >
      ...
      {stats && stats.totalSessions > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          {[
            { label: t('stats.totalSessions'), value: stats.totalSessions, color: ACCENT.reading, icon: <IconBookOpen size={20} /> },
      ...
      <div className="flex flex-col gap-5 mb-10">
        ...
        <div className="flex gap-3 overflow-x-auto no-scrollbar pb-1">
          {[
            ...
            { id: 'DRAFT', label: tCommon('status.DRAFT') },
            { id: 'GRADED', label: tCommon('status.GRADED') },
          ].map(status => {
            const isActive = filterStatus === status.id;
            const visual = STATUS_CONFIG_VISUAL[status.id];
            const color = status.id === '' ? ACCENT.reading : visual?.color || ACCENT.reading;
            return (
      ...
      {isLoading ? (
        ...
        <div className="text-center py-28 rounded-[3.5rem] border-2 border-dashed" style={{ borderColor: 'var(--theme-border)' }}>
          ...
          <Link href="/practice-test/reading/new" className="inline-flex items-center gap-2 px-10 py-5 rounded-2xl text-base font-black text-white shadow-2xl shadow-emerald-500/30 transition-all hover:scale-105 active:scale-95"
            style={{ background: GRADIENT.reading }}>{t('emptyCta')}</Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 gap-8">
          {history.data.map(item => {
            const knownTextTypes = ['anzeige', 'kurznachricht', 'brief', 'artikel', 'dialog'];
            const typeLabelVi = knownTextTypes.includes(item.textType)
              ? t(`textTypes.${item.textType}` as 'textTypes.anzeige')
              : item.textType;
            const statusKey = STATUS_LABEL_KEY[item.status] ?? 'DRAFT';
            ...
            const statusVisual = (STATUS_CONFIG_VISUAL[statusKey] ?? STATUS_CONFIG_VISUAL.DRAFT)!;
            const statusLabel = tCommon(`status.${statusKey}` as 'status.DRAFT');
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                      ...
                      <span className="text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-lg border bg-black/3 dark:bg-white/5"
                        style={{ borderColor: 'var(--theme-border)', color: 'var(--theme-text-muted)' }}>{typeLabelVi}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-3 text-[11px] font-bold opacity-40 uppercase tracking-widest" style={{ color: 'var(--theme-text-primary)' }}>
                      <span>{t('questionsCount', { count: item.totalQuestions })}</span>
                      {item.status === 'GRADED' && (
                        <>
                          <span className="w-1 h-1 rounded-full bg-current" />
                          <span className="text-emerald-500">{t('correctCount', { correct: item.correctCount, total: item.totalQuestions })}</span>
                        </>
                      )}
                      <span className="w-1 h-1 rounded-full bg-current" />

...
export default function SpeakingRoomsListPage() {
  ...
  const handleJoinByCode = async () => {
    ...
    try {
      ...
      setCodeError(t('list.invalidCode'));
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="space-y-3">
        ...
        {isLoading && <p style={{ color: 'var(--theme-text-muted)' }}>{t('list.loading')}</p>}
        {!isLoading && rooms && rooms.length === 0 && (
          <div className="py-8 text-center rounded-2xl" style={{ backgroundColor: 'var(--theme-bg-card)', border: '1px dashed var(--theme-border)' }}>
            <p style={{ color: 'var(--theme-text-muted)' }}>{t('list.empty')}</p>
          </div>
        )}
        {rooms?.map((room) => (

...
type TaskTypeKey = 'richtig_falsch' | 'multiple_choice' | 'zuordnung';

function useTaskTypeLabel() {
  const t = useTranslations('practice.examListening.result.taskTypes');
  return (type: string) => {
    const keys: TaskTypeKey[] = ['richtig_falsch', 'multiple_choice', 'zuordnung'];
          ...
          {texts.map(text => (
            <div key={text.id}>
              ...
              <button onClick={() => speakText(text.content)}
                ...
                <IconVolume2 size={11} style={{ color: ACCENT.listening }} /> {t('listenAgain')}
              </button>
            </div>
          ))}
        </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                {!detail.isCorrect && (
                  <p className="text-xs mt-0.5" style={{ color: STATUS.danger }}>
                    {tCommon('userVsCorrect', { user: detail.userAnswer ?? tCommon('emDash'), correct: detail.correctAnswer })}
                  </p>
                )}
              </div>
                ...
                <div className="flex gap-3 flex-wrap">
                  <div className="flex items-center gap-1.5 text-xs">
                    ...
                    <span className="font-bold px-2 py-0.5 rounded-lg"
                      ...
                      {detail.userAnswer ?? tCommon('skipped')}
                    </span>
                  </div>
                  {!detail.isCorrect && (
                    <div className="flex items-center gap-1.5 text-xs">
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm" style={{ color: 'var(--theme-text-muted)' }}>{tCommon('loadingResult')}</p>
        </div>
      </div>
    );
  }
        ...
        <div className="px-5 pt-4 pb-3 border-b text-center" style={{ borderColor: 'var(--theme-border)' }}>
          ...
          {session.submittedAt && (
            <p className="text-caption" style={{ color: 'var(--theme-text-muted)' }}>
              {fmt.dateTime(new Date(session.submittedAt), { dateStyle: 'short', timeStyle: 'short' })}
            </p>
          )}
        </div>
        ...
        <div className="flex flex-col sm:flex-row">
          ...
          <div className="sm:w-1/2 flex flex-col justify-center p-5 gap-3">
            ...
            <p className="text-body" style={{ color: 'var(--theme-text-secondary)' }}>
              {tCommon('gesamtergebnis')}: <strong style={{ color: 'var(--theme-text-primary)' }}>
                {session.correctCount} / {session.totalQuestions}
              </strong> {tCommon('richtigSuffix')}
            </p>
            <div className="grid grid-cols-2 gap-2 mt-1">
              {[
                { label: tCommon('correctLabel'), value: session.correctCount,                                          color: STATUS.success, bg: `${STATUS.success}1A` },
                { label: tCommon('wrongLabel'),  value: (session.totalQuestions ?? 0) - (session.correctCount ?? 0),  color: STATUS.danger,  bg: `${STATUS.danger}1A` },
              ].map(s => (
                ...
                <div key={ts.teil}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-body font-semibold" style={{ color: 'var(--theme-text-primary)' }}>
                        {teil ? taskTypeLabel(teil.taskType) : t('teilFallback', { n: ts.teil })}
                      </span>
                    </div>

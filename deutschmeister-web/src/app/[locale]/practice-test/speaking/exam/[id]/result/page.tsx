...
const CRITERIA_VISUAL = {
  aufgabe:    { icon: '🎯', color: STATUS.success },
  grammatik:  { icon: '📐', color: ACCENT.xp      },
  aussprache: { icon: '🔊', color: ACCENT.cyan    },
  wortschatz: { icon: '📚', color: ACCENT.vocab   },
};

function useGrade() {
  const t = useTranslations('practice.examSpeaking.result');
  return (score: number) => {
    if (score >= 80) return { badge: t('gradeBadgeExcellent'),  emoji: '🏆', title: t('gradeTitleExcellent'),   color: STATUS.success, bg: 'rgba(34,197,94,.15)'  };
    if (score >= 65) return { badge: t('gradeBadgeGood'),       emoji: '🌟', title: t('gradeTitleGood'),        color: STATUS.success, bg: 'rgba(34,197,94,.12)'  };
    if (score >= 60) return { badge: t('gradeBadgePassed'),     emoji: '✓',  title: t('gradeTitlePassed'),      color: ACCENT.xp,     bg: 'rgba(245,158,11,.15)' };
    if (score >= 40) return { badge: t('gradeBadgeKeepTrying'), emoji: '📖', title: t('gradeTitleKeepTrying'),  color: ACCENT.games,  bg: 'rgba(249,115,22,.12)' };
    return             { badge: t('gradeBadgeFailed'),       emoji: '💪', title: t('gradeTitleFailed'),      color: STATUS.danger, bg: 'rgba(239,68,68,.12)'  };
  };
}
...
function TeilDetailCard({ teil, grading, transcript }: {
  ...
  const t = useTranslations('practice.examSpeaking.result');
  const tCriteria = useTranslations('practice.examSpeaking.result.criteriaLabels');
                ...
                <div className="rounded-xl p-4"
                  ...
                  <div className="space-y-3">
                    {CRITERIA_ORDER.map(key => {
                      const cfg = CRITERIA_VISUAL[key];
                      const val = grading.criteriaScores?.[key] ?? 0;
                      return (
                        <div key={key}>
                          <div className="flex items-center gap-1.5 mb-1.5">
                            <span className="text-body">{cfg.icon}</span>
                            <span className="text-xs font-bold flex-1" style={{ color: 'var(--theme-text-primary)' }}>{tCriteria(key)}</span>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="rounded-xl p-3" style={{ backgroundColor: 'var(--theme-bg-secondary)', border: `1px solid ${STATUS.success}33` }}>
                      <div className="flex items-center gap-1.5 mb-2">
                        ...
                        <span className="text-caption font-semibold" style={{ color: 'var(--theme-text-secondary)' }}>{t('nativeSpeaker')}</span>
                      </div>
                      ...
                      return (
                        <div key={i} className="rounded-xl p-3"
                          ...
                          <p className="text-caption font-bold mb-1" style={{ color: isPron ? ACCENT.xp : STATUS.danger }}>
                            {i + 1} · {isPron ? t('errorPronunciation') : t('errorGrammar')}
                          </p>
                ...
                {(strengths.length > 0 || corrections.length > 0) && (
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div className="rounded-xl p-3" style={{ background: 'rgba(245,158,11,.05)', border: `1px solid ${ACCENT.xp}33` }}>
                      <p className="text-caption font-bold mb-2 flex items-center gap-1" style={{ color: ACCENT.xp }}>{t('improvementsTitle')}</p>
                      <ul className="space-y-1.5">
...
export default function ExamSpeakingResultPage() {
  ...
  if (isLoading) return (
    <div className="py-16 flex flex-col items-center gap-3">
      ...
      <p className="text-body" style={{ color: 'var(--theme-text-muted)' }}>{t('loadingResult')}</p>
    </div>
  );

  if (!session) return (
    <div className="py-10 text-center">
      <p className="mb-3" style={{ color: 'var(--theme-text-muted)' }}>{t('examNotFound')}</p>
      <Link href="/practice-test/speaking/exam" className="text-body" style={{ color: ACCENT.xp }}>{t('backToList')}</Link>
    </div>
  );
  ...
  if (session.status === 'GRADING') return (
    <div className="py-16 flex flex-col items-center gap-5">
      ...
      <div className="text-center">
        <p className="font-bold text-[15px]" style={{ color: 'var(--theme-text-primary)' }}>{t('aiGrading')}</p>
        <p className="text-body mt-1" style={{ color: 'var(--theme-text-muted)' }}>{t('aiGradingBody')}</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="max-w-360 mx-auto px-4 py-6">
      ...
      <div className="flex items-center gap-2.5 mb-5">
        ...
        <div>
          <p className="text-caption font-medium" style={{ color: 'var(--theme-text-muted)' }}>{t('kicker')}</p>
          <h1 className="text-base font-bold leading-tight" style={{ color: 'var(--theme-text-primary)' }}>
            {t('headerSkill', { examType: session.examType, cefrLevel: session.cefrLevel })}
          </h1>
        </div>
      </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 flex-wrap mt-2">
            <span className="text-xs" style={{ color: 'var(--theme-text-muted)' }}>
              {t('teileCount', { count: teile.length, cefrLevel: session.cefrLevel })}
            </span>
            {session.gradedAt && (
              <span className="text-caption" style={{ color: 'var(--theme-text-muted)' }}>
                · {fmt.dateTime(new Date(session.gradedAt), { dateStyle: 'short' })}
              </span>
            )}
          </div>
        </div>

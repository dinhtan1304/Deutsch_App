        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-lg"
              ...
              {isGraded ? tCommon('statusGraded') : isGrading ? tCommon('statusGrading') : tCommon('statusDraft')}
            </span>
          </div>
    ...
    >
      ...
      {stats && stats.total > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          {[
            { label: tCommon('stats.total'), value: stats.total, color: ACCENT.xp, icon: <IconMic size={20} /> },
      ...
      {isLoading ? (
        ...
        <div className="text-center py-28 rounded-[3.5rem] border-2 border-dashed" style={{ borderColor: 'var(--theme-border)' }}>
          ...
          <Link href="/practice-test/speaking/exam/new"
            ...
            style={{ background: GRADIENT.speaking }}>{tCommon('startNow')}</Link>
        </div>
      ) : (

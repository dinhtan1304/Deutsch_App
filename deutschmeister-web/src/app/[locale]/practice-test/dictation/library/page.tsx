        ...
        {imgError ? (
          <div className="w-full h-full flex items-center justify-center bg-white/2">
            <div className="flex flex-col items-center gap-2 opacity-20">
              ...
              <span className="text-[10px] font-black uppercase tracking-tighter">{t('previewUnavailable')}</span>
            </div>
          </div>
        ) : (
          <Image
    ...
    <div className="py-6 space-y-10">
        ...
        {data?.availableTopics && data.availableTopics.length > 0 && (
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-2">
            ...
            {data.availableTopics.map(topicItem => (
              <button key={topicItem} onClick={() => { setTopic(topicItem); setPage(1); }}
                className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest whitespace-nowrap border ${topic === topicItem ? 'border-cyan-500/50 text-cyan-500 bg-cyan-500/5' : 'border-transparent text-muted hover:text-primary'}`}>
                {topicItem}
              </button>
            ))}
          </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-32 rounded-[3rem] border-2 border-dashed transition-all hover:border-cyan-500/30" 
          ...
          <p className="text-sm font-black uppercase tracking-widest opacity-30">{t('empty')}</p>
        </div>
      ) : (
        <>

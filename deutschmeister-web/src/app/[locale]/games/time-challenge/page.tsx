...
export default function TimedChallengePage() {
  ...
  const startGame = async () => {
    ...
    try {
      ...
      if (isWordBankMode) {
        ...
        if (eligible.length < 10) { setLoadError(t('timeChallenge.errors.minNouns10')); setStarting(false); return; }
      ...
      if (!firstBatch.length) { setLoadError(t('common.errors.noVocab')); setStarting(false); return; }
      setWords(firstBatch);
      setPhase('countdown');
    ...
    } catch { setLoadError(t('timeChallenge.errors.loadFailed')); isFetchingRef.current = false; }
  ...
  if (phase === 'setup') {
    return (
        <GameSetupCard icon={({ size }) => <IconClock size={size} style={{ color: 'white' }} />} iconColor={STATUS.danger} title={t('timeChallenge.title')} loadError={loadError}>
          <p className="text-sm mb-1" style={{ color: 'var(--theme-text-secondary)' }}>
            {t.rich('timeChallenge.description', {
              seconds: duration,
              emphasis: (chunks) => <span className="font-bold" style={{ color: STATUS.danger }}>{chunks}</span>,
            })}
          </p>
          <p className="text-xs mb-6" style={{ color: 'var(--theme-text-muted)' }}>{t('timeChallenge.settingsHint')}</p>
          <GameInfoBox>
            <div className="flex items-center gap-2"><IconTarget size={14} style={{ color: STATUS.danger }} /><span>{t('timeChallenge.info.scoring')}</span></div>
            <div className="flex items-center gap-2"><IconKeyboard size={14} style={{ color: ACCENT.vocab }} /><span>{t.rich('timeChallenge.info.hotkeys', { kbd: (chunks) => <KBD>{chunks}</KBD> })}</span></div>
            <div className="flex items-center gap-2"><IconVolume size={14} style={{ color: STATUS.success }} /><span>{t('timeChallenge.info.sound', { state: settings.soundEnabled ? t('common.soundOn') : t('common.soundOff') })}</span></div>
          </GameInfoBox>
          <div className="flex gap-3 justify-center mt-6">
            <Button variant="game" accent="games" onClick={startGame} isLoading={starting}><IconRocket size={16} /> {t('common.start')}</Button>
    ...
    return (
        <div className="flex items-center justify-center py-20">
          <div className="text-center">
            ...
            <p className="text-lg font-semibold" style={{ color: 'var(--theme-text-muted)' }}>{t('timeChallenge.getReady')}</p>
          </div>
        </div>
    );
  }
        ...
        <div className="flex justify-between items-center mb-4">
          {combo >= 2 ? (
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold text-white"
              ...
              {t('timeChallenge.comboLabel', { mult: Math.min(combo, 4) })}
            </span>
          ) : <span />}

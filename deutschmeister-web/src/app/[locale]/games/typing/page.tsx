...
export default function TypingPracticePage() {
  ...
  const startGame = async () => {
    ...
    try {
      ...
      if (list.length === 0) {
        setLoadError(t('typing.errors.noSentences'));
        return;
      }
      setSentences(list);
      ...
      setLoadError(t('typing.errors.loadFailed'));
    } finally {
    ...
    return (
      ...
      >
        <p className="text-sm mb-1" style={{ color: 'var(--theme-text-secondary)' }}>
          {t.rich('typing.description', {
            seconds: DURATION_SEC,
            emphasis: (chunks) => (
              <span className="font-bold" style={{ color: ACCENT.games }}>{chunks}</span>
            ),
          })}
        </p>
        ...
        <GameInfoBox>
          <div className="flex items-center gap-2">
            ...
            <span>{t('typing.info.duration', { seconds: DURATION_SEC })}</span>
          </div>
          <div className="flex items-center gap-2">
            <IconKeyboard size={14} style={{ color: ACCENT.games }} />
            <span>
              {t.rich('typing.info.controls', { kbd: (chunks) => <KBD>{chunks}</KBD> })}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <IconTarget size={14} style={{ color: ACCENT.games }} />
            <span>
              {t('typing.info.umlautHint', { hint: '' })}
        ...
        <Button
          ...
          className="mt-5"
        >
          {loadingSentences ? t('typing.loadingSentences') : t('typing.startCta')}
        </Button>
      </GameSetupCard>
    );
  }
    ...
    return (
      ...
      >
        ...
        <p
          ...
          style={{ color: 'var(--theme-text-muted)' }}
        >
          {t('typing.countdownReady', { count: sentences.length })}
        </p>
      </div>
    );
  }
      ...
      <>
        <GameResultCard accuracy={accuracy} title={t('common.completed')}>
          ...
          <div className="grid grid-cols-2 gap-3 mb-5">
            ...
            />
            <StatCard
              label={t('typing.stats.sentencesCompleted')}
              value={sentencesCompleted}
              color={ACCENT.srs}
              icon={IconCheck}
            />
            <StatCard
              label={t('typing.stats.errorsLabel')}
  ...
  return (
    ...
    >
      <GamePlayHeader
        title={t('typing.title')}
        subtitle={t('typing.play.satzLabel', {
          current: sentenceIdx + 1,
          total: sentences.length,
          category: TYPING_CATEGORY_LABELS[category].de.toUpperCase(),
        })}
        timer={timerStr}
        onExit={() => {
          clearTimers();
      ...
      <GameStatsBar
        stats={[
          { label: t('typing.stats.wpm'), value: liveWpm, color: ACCENT.games, dot: true },
          { label: t('typing.stats.accuracy'), value: `${liveAccuracy}%`, color: STATUS.success, dot: true },
          { label: t('typing.stats.sentenceShort'), value: sentencesCompleted, color: ACCENT.srs, dot: true },
          { label: t('typing.stats.errorsLabel'), value: incorrectChars, color: STATUS.danger, dot: true },
        ]}
      />
      ...
      <input
        ...
        autoCorrect="off"
        spellCheck={false}
        aria-label={t('typing.play.ariaInput')}
        style={{
          position: 'fixed',
          top: 0,
          left: 0,
      ...
      <p
        ...
        style={{ color: 'var(--theme-text-muted)' }}
      >
        <span className="md:hidden">{t('typing.play.tapToFocus')}</span>
        {t('typing.play.controls', { seconds: DURATION_SEC })}
        <span className="font-mono">{UMLAUT_TRIGGER_HINT}</span>
      </p>
    </div>
  );

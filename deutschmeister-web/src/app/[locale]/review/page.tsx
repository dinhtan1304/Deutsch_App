    ...
    <div className="max-w-2xl mx-auto py-6">
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-caption font-medium" style={{ color: 'var(--theme-text-muted)' }}>{t('sessionLabel')}</p>
            <p className="text-[15px] font-bold leading-tight" style={{ color: 'var(--theme-text-primary)' }}>
              {currentMode === 'gender' ? t('modeGender') : currentMode === 'de-vi' ? t('modeDeVi') : currentMode === 'vi-de' ? t('modeViDe') : t('modeReview')}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-4 gap-1 mb-5 rounded-2xl p-3.5"
        ...
        {[
          { label: t('statTotal'),   value: reviewQueue.length,            color: 'var(--theme-text-primary)', dot: false },
          { label: t('statNew'),    value: queueNew,                      color: ACCENT.srs, dot: true },
          { label: t('statDue'), value: reviewQueue.length - queueNew, color: ACCENT.xp, dot: true },
          { label: t('statKnown'),  value: sessionStats.correct,          color: STATUS.success, dot: true },
        ].map(s => (

...
export function SRSCompleteScreen({ sessionStats, stats, onContinue, onAddWords, onBack }: SRSCompleteScreenProps) {
  ...
  const resultItems = [
    { label: t('resultCorrect'),      value: sessionStats.correct,       color: STATUS.success, bg: `linear-gradient(135deg, ${STATUS.success}1F, ${STATUS.success}0F)` },
    { label: t('resultWrong'),       value: sessionStats.wrong,         color: STATUS.danger,  bg: `linear-gradient(135deg, ${STATUS.danger}1F, ${STATUS.danger}0F)` },
    { label: t('resultAccuracy'), value: `${accuracy}%`,             color: ACCENT.srs,     bg: `linear-gradient(135deg, ${ACCENT.srs}1F, ${ACCENT.srs}0F)` },
    { label: t('resultBestStreak'), value: sessionStats.bestStreak,  color: ACCENT.games,   bg: `linear-gradient(135deg, ${ACCENT.games}1F, ${ACCENT.games}0F)` },
  ];

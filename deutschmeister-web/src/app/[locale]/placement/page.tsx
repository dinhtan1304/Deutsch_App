...
type SublevelKey =
  | 'sublevelA1Foundation' | 'sublevelA1Begin'
  | 'sublevelA2Upper' | 'sublevelA2Lower'
  | 'sublevelB1Upper' | 'sublevelB1Lower';

function determineSublevel(a1: number, a2: number, b1: number): { level: 'A1' | 'A2' | 'B1'; sublabelKey: SublevelKey } {
  const base = determineLevel(a1, a2, b1);
  if (base === 'A1') {
    return a1 >= 4
      ? { level: 'A1', sublabelKey: 'sublevelA1Foundation' }
      : { level: 'A1', sublabelKey: 'sublevelA1Begin' };
  }
  if (base === 'A2') {
    return a2 >= 4
      ? { level: 'A2', sublabelKey: 'sublevelA2Upper' }
      : { level: 'A2', sublabelKey: 'sublevelA2Lower' };
  }
  return b1 >= 4
    ? { level: 'B1', sublabelKey: 'sublevelB1Upper' }
    : { level: 'B1', sublabelKey: 'sublevelB1Lower' };
}

type Phase = 'intro' | 'quiz' | 'result';

export default function PlacementRetakePage() {
  const t = useTranslations('onboarding.placement');
  ...
  const { level, sublabelKey } = useMemo(() => determineSublevel(scores.a1, scores.a2, scores.b1), [scores]);
          ...
          <div className="rounded-2xl border p-8 text-center"
            ...
            {settings?.preferredLevel && (
              <p className="text-xs mb-4 px-3 py-1.5 rounded-lg inline-block"
                ...
                {t('currentLevel', { level: settings.preferredLevel })}
              </p>
            )}
        ...
        {phase === 'quiz' && (
          <div className="rounded-2xl border p-6" style={{ borderColor: 'var(--theme-border)', backgroundColor: 'var(--theme-bg-card)' }}>
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs" style={{ color: 'var(--theme-text-muted)' }}>{t('progressFormat', { current: qIdx + 1, total: QUESTIONS.length })}</span>
            </div>
        ...
        {phase === 'result' && (
          <div className="rounded-2xl border p-8 text-center"
            ...
            <p className="text-body mb-5" style={{ color: 'var(--theme-text-muted)' }}>
              {t('resultScore', { correct: scores.total })}
            </p>
            ...
            <div className="flex gap-3 justify-center">
              ...
              <button onClick={handleSave} disabled={saving}
                ...
                {saving ? t('saving') : t('updateTo', { level })}
              </button>
            </div>
          </div>
        )}

      ...
      <div className="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl"
        ...
        <div className="sticky top-0 z-10 flex items-center justify-between p-4 border-b"
          ...
          <button onClick={() => { reset(); onClose(); }}
            className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 text-xl">✕</button>
        </div>
          ...
          {step === 'upload' && (
            <div className="space-y-5">
              <div className="p-4 rounded-xl border" style={{ borderColor: 'var(--theme-border)', backgroundColor: 'var(--theme-bg-secondary)' }}>
                ...
                <p className="text-sm text-gray-500 mb-3">Từ đã có trong sổ sẽ được <strong>tự động bỏ qua</strong>.</p>
                <table className="text-xs w-full">
                  ...
                  <tbody className="text-gray-500">
                    <tr><td className="p-1.5 font-mono">word</td><td>Từ gốc</td><td className="text-red-500 font-bold">✱</td></tr>
                    <tr><td className="p-1.5 font-mono">wordType</td><td>nomen / verb / adjektiv / ...</td><td className="text-red-500 font-bold">✱</td></tr>
                    <tr><td className="p-1.5 font-mono">article</td><td>der / die / das</td><td className="text-red-500">✱ nomen</td></tr>
                    <tr><td className="p-1.5 font-mono">translationEn</td><td>Nghĩa tiếng Anh</td><td className="text-red-500 font-bold">✱</td></tr>
                    <tr><td className="p-1.5 font-mono">translationVi</td><td>Nghĩa tiếng Việt</td><td className="text-red-500 font-bold">✱</td></tr>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-bold" style={{ color: 'var(--theme-text-primary)' }}>
                  ...
                  <span className="text-sm text-gray-400 font-normal ml-2">({fileName})</span>
                </h3>
                <button onClick={reset} className="text-sm text-blue-500 hover:underline">← Chọn file khác</button>
              </div>

              {importError && <div className="p-3 rounded-xl bg-red-50 border border-red-300"><p className="text-sm text-red-600">❌ {importError}</p></div>}

              {previewErrors.length > 0 && (
                <div className="p-3 rounded-xl bg-yellow-50 border border-yellow-300">
                  <p className="font-medium text-yellow-700 mb-2">⚠️ {previewErrors.length} cảnh báo</p>
                  <div className="max-h-28 overflow-y-auto space-y-1">
                    {previewErrors.slice(0, 20).map((err, i) => (
                      <p key={i} className="text-xs text-yellow-600">Dòng {err.row}: [{err.field}] {err.message}</p>
                    ))}
                  </div>
                </div>
              )}
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr style={{ backgroundColor: 'var(--theme-bg-secondary)' }}>
                      ...
                      <th className="p-2 text-left text-xs font-medium text-gray-500">Từ</th>
                      <th className="p-2 text-left text-xs font-medium text-gray-500">Loại</th>
                      <th className="p-2 text-left text-xs font-medium text-gray-500">EN</th>
                      <th className="p-2 text-left text-xs font-medium text-gray-500">VI</th>
                      <th className="p-2 text-left text-xs font-medium text-gray-500">Level</th>
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      return (
                        <tr key={i} className="border-t" style={{ borderColor: 'var(--theme-border)', backgroundColor: hasCritical ? 'rgba(239,68,68,0.08)' : undefined }}>
                          ...
                          <td className="p-2 font-medium" style={{ color: 'var(--theme-text-primary)' }}>
                            {row.article && <span className="text-gray-400 text-sm">{row.article} </span>}{row.word}
                            {hasCritical && <span className="ml-1 text-red-500">⚠</span>}
                          </td>
                          ...
                          <td className="p-2 text-gray-500 text-sm max-w-[150px] truncate">{row.translationEn}</td>
                          <td className="p-2 text-gray-500 text-sm max-w-[150px] truncate">{row.translationVi}</td>
                          <td className="p-2 text-gray-400 text-xs">{row.level || 'A1'}</td>
                        </tr>
                      );
                    })}
                  </tbody>
                </table>
                {parsedRows.length > 50 && <p className="p-2 text-center text-xs text-gray-400">...và {parsedRows.length - 50} từ khác</p>}
              </div>
            ...
            <div className="space-y-5 py-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="p-4 rounded-xl text-center" style={{ backgroundColor: 'rgba(34,197,94,0.1)' }}>
                  ...
                  <div className="text-sm text-green-600 font-medium">✅ Đã thêm</div>
                </div>
                <div className="p-4 rounded-xl text-center" style={{ backgroundColor: 'rgba(234,179,8,0.1)' }}>
                  ...
                  <div className="text-sm text-yellow-600 font-medium">⏭️ Đã có</div>
                </div>
              ...
              {importResult.skippedWords && importResult.skippedWords.length > 0 && (
                <div className="p-3 rounded-xl border" style={{ borderColor: 'var(--theme-border)', backgroundColor: 'var(--theme-bg-secondary)' }}>
                  <p className="text-sm font-medium text-yellow-600 mb-2">⏭️ Từ đã có (bỏ qua):</p>
                  <div className="flex flex-wrap gap-1.5 max-h-24 overflow-y-auto">
                    {importResult.skippedWords.slice(0, 30).map((w, i) => (
                      <span key={i} className="px-2 py-1 rounded-lg text-xs font-medium bg-yellow-100 text-yellow-700">{w}</span>
                    ))}
                  </div>
                </div>
              )}
